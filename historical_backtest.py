...
class _Stats:
    def __init__(self):
        self.n = 0          # with result
        self.wins = 0
        self.places = 0     # top 3
        self.pl = 0.0       # level-stakes P/L at SP
        self.no_result = 0  # no result data
...
def _render_table(breakdown: dict[str, _Stats], sort_key="wins") -> list[str]:
    """Return lines for a breakdown table, sorted by win_pct descending."""
    header = f"  {'Category':<26} {'Sels':>5} {'Wins':>5} {'Win%':>6} {'Plc%':>6} {'P/L':>8} {'ROI%':>7}"
    sep    = "  " + "-" * 62
    rows   = [header, sep]
    items  = sorted(
        [(k, v) for k, v in breakdown.items() if v.n > 0],
        key=lambda kv: -kv[1].win_pct,
    )
    for name, s in items:
        rows.append(
            f"  {name:<26} {s.n:>5} {s.wins:>5} "
