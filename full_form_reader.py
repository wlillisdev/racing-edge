...
def _enrich_profile(
    ...
    if wins:
        best_ground = _result_ground(wins[0])
    elif results:
    ...
    for r in wins:
        ...
        if r_class_int <= today_class_int:
...
def main() -> int:
    ...
    # Flat lookup keyed by horse_id — used by nap_selector_v3.py
    horses_flat = {
        hid: profile.get("results", [])
        for hid, profile in all_profiles.items()
    }
