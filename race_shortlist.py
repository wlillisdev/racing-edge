...
EXCLUDED_TYPES: frozenset[str] = frozenset()   # e.g. frozenset({"Bumper"})
...
def _best_morning_price(runner: dict) -> float | None:
    """Return the best (lowest) decimal price from bookmaker odds array.

    The Pro racecard returns odds as a list of {bookmaker, decimal, ...}.
...
def _shortlist_race(race: dict) -> tuple[bool, list[dict]]:
    """Return (race_passes, list_of_enriched_runners)."""
