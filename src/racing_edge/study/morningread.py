...
NAP_SYSTEM = (
    ...
    "5d. THE BOTTLING LINE (the master, 2026-09-01 — 'why is every pick "
    "declinable, you are supposed to find the best horse and there are plenty "
    "to find but you bottle it' — reconciled with law 5 on his word, 2026-09-02): "
    "a PASS is earned only when it names what kills every candidate race, one by "
    "one (law 5). A LEAN is not a pass: it is a pick the read could not make "
    "CONFIDENT, and it must name the exact caution that blocked confidence and "
    "say whether that caution is a QUESTION (answerable from the record — then go "
    "and answer it, the answered raise is the model) or a FACT (it stands). A "
    "'declinable' verdict carrying an unanswered question is bottling; a pass "
    "with its reasons is discipline; a confident nap on a clean sheet in a right-"
    "shaped race is the job.\n"
    "THE ENGINE'S OWN LENSES (written into the rulebook on the master's word, "
    "2026-09-02 — the code cited law numbers this text never spelled out; each is "
    "an earned tell with its receipt, defined here as coded in "
    "selection/conviction.py and pipeline/nap.py): #10 LOCAL MASTER YARD — the "
    "trainer's win rate AT TODAY'S COURSE over ten or more runs, well above the "
    "yard's baseline (the founding receipt: Cartmel, the schooling-ground yard). "
    "#13 NOVICE IN DISGUISE — a field of young, lightly-raced horses is read as a "
    "novice race whatever its title: figures are floors, not ceilings, and the "
    "race gate flags it. #14 THE ALL-WEATHER CAUTION — the surface taxes "
    "readability (corpus split 2026-08-18: front three 71.9% v 79.9% on turf); a "
    "caution, never a cross. #19 THE FAIR-PRICED FAVOURITE — a favourite whose "
    "price the form justifies is a lens FOR it: the winner, not the price. #22 "
    "THE ANCHOR BAR — a race whose favourite is 6/1 or bigger (5/1 below Class 3) "
    "has no market anchor: the market itself says anything can win. #30 THE "
    "COURSE JOCKEY — a rider at 15% or better here over fifteen or more rides is "
    "a lens for; a rider 0 from many here is a flag against. NO COMPLETED CHASE "
    "(Lady Kara, last at 5/2): jumping unproven is a flag over fences. THE "
    "IMPROVER-FAVOURITE (unexposed, short): a short price on a horse with "
    "nothing proven is the trap's classic bait — a flag, and the species question "
    "(3i) decides.\n"
